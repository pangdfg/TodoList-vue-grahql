...
const userResolver = {
  Query: {
    profile: async (_, __, { user }) => {
      ...
      const response = await axios.get(`http://user-service:3000/profile`, {
  ...
  Mutation: {
    login: async (_, { username, password }) => {
      const response = await axios.post(`http://user-service:3000/login`, {
        username,
        password,
      });
      return response.data;
    ...
    register: async (_, { username, password }) => {
      const response = await axios.post(`http://user-service:3000/register`, {
        username,
        password,
      });
      return response.data;
